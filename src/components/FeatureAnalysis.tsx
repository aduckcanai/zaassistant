  ...
  const getInitialAnalysisData = (): AnalysisData => {
    ...
    if (isProductAnalysisLoaded && productAnalysisData) {
      return {
        ...
        scope: {
          value: productAnalysisData.scope
            ? `In Scope:\n${productAnalysisData.scope.in_scope?.join('\n') || ''}\n\nOut of Scope:\n${productAnalysisData.scope.out_scope?.join('\n') || ''}\n\nConstraints:\n${productAnalysisData.scope.constraints?.join('\n') || ''}`
            : '',
          isAIGenerated: true,
        },
        successMetrics: {
          value: productAnalysisData.success_metrics
            ?.map(metric => `${metric.name} (${metric.type}): ${metric.formula} - Target: ${metric.target}`)
            .join('\n\n') || '',
          isAIGenerated: true,
        },
      };
    ...
    targetUsers: { value: '', isAIGenerated: false },
      ...
      scope: { value: '', isAIGenerated: false },
      successMetrics: { value: '', isAIGenerated: false },
    };
  };
...
Project Scope
${analysisData.scope?.value || '[Not filled]'}
...
${analysisData.successMetrics?.value || '[Not filled]'}
      ...
      <div className='bg-card/80 backdrop-blur-sm rounded-xl border border-border/50 p-8 lg:p-10 space-y-12 shadow-lg'>
        ...
        <div className='space-y-8'>
          <div className='border-b border-border/50 pb-4'>
            ...
            <p className='text-sm text-muted-foreground mt-2 leading-relaxed'>
              Defining what is included and excluded from your feature
            </p>
          </div>
          ...
          <div className='space-y-8'>
            <FieldComponent
              ...
              title='Project Scope'
              field='scope'
              description='What features, functionality, and deliverables are included or excluded from this project? Include constraints and limitations.'
              placeholder='e.g., In Scope: Core task management, user authentication, mobile app...\n\nOut of Scope: Advanced reporting, third-party integrations...\n\nConstraints: Must work offline, limited to 1000 tasks per user...'
              value={analysisData.scope.value}
              onChange={value => handleFieldUpdate('scope', value)}
              isAIGenerated={analysisData.scope.isAIGenerated}
              onMagicEdit={instruction =>
                handleMagicEdit('scope', instruction)
              }
              isEditing={isEditing}
              magicEditField={magicEditField}
              setMagicEditField={setMagicEditField}
        ...
        <div className='space-y-8'>
          <div className='border-b border-border/50 pb-4'>
            ...
            <p className='text-sm text-muted-foreground mt-2 leading-relaxed'>
              Measuring the success and impact of your feature
            </p>
          </div>
          ...
          <div className='space-y-8'>
            <FieldComponent
              ...
              description='What metrics will you use to measure success? Include engagement, retention, revenue, or operational metrics with specific targets.'
              placeholder='e.g., User Engagement: Daily active users - Target: 25% increase\n\nTask Completion Rate: % of tasks completed within 24 hours - Target: 80%\n\nRevenue Impact: Premium conversion rate - Target: 15%...'
              value={analysisData.successMetrics.value}
              onChange={value => handleFieldUpdate('successMetrics', value)}
              isAIGenerated={analysisData.successMetrics.isAIGenerated}
              onMagicEdit={instruction =>
                handleMagicEdit('successMetrics', instruction)
              }
              isEditing={isEditing}
              magicEditField={magicEditField}
              setMagicEditField={setMagicEditField}
